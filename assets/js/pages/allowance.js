...
const AllowancePage = {
  ...
  render() {
    ...
    const monthTotal = DB.data.allowances.filter((a) => Utils.inRange(a.date, 'monthly')).reduce((s, a) => s + a.amount, 0);
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ${items.map((a) => `
              <tr>
                <td><strong>${Utils.escapeHtml(a.source)}</strong></td>
                <td class="text-secondary">${Utils.escapeHtml(a.notes) || '—'}</td>
                <td class="text-secondary">${Utils.formatDateTime(a.date, a.time)}</td>
                <td><span class="activity-amount pos">${Utils.formatMoney(a.amount)}</span></td>
                <td>
  ...
  handleSubmit(e) {
    ...
    const payload = { amount: amountEl.value, source, notes, date, time };

    if (this.editingId) {
      DB.updateAllowance(this.editingId, payload);
      Toast.success('Allowance updated', `${source} · ${Utils.formatMoney(payload.amount)}`);
    } else {
      DB.addAllowance(payload);
      Toast.success('Allowance added', `${source} · ${Utils.formatMoney(payload.amount)}`);
    }
  ...
  async remove(id) {
    ...
    const ok = await confirmDialog({
      ...
      message: `This will remove ${Utils.formatMoney(a.amount)} from ${a.source} and update your balance. This can't be undone.`,
      confirmText: 'Delete'
    });
    if (!ok) return;
    DB.deleteAllowance(id);
    Toast.success('Allowance deleted', 'Your balance has been updated.');
  }
};
