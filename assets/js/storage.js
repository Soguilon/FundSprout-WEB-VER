...
class StorageService {
    ...
    try {
      switch (event) {
        case 'allowance-added':
          SyncManager.queueOperation('allowances', 'CREATE', payload.id, payload); break;
        case 'allowance-updated':
          SyncManager.queueOperation('allowances', 'UPDATE', payload.id, payload); break;
  ...
  _recalculateBalance() {
    ...
    const allowances = this.data.allowances.reduce((sum, a) => sum + (Number(a.amount) || 0), 0);
